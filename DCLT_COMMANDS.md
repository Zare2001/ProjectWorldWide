...
| thing | state |
|---|---|
| VM full-arm stack (29510/11) | LIVE and **FRESH** — clean reset 13:43, run6, 0 checkpoints, NEW token |
| full-arm site jobs | none should exist; scancel any leftover `*titan-full*` (§1) |
| VM dclt stack (29540/41) | LIVE with QSR + pure-average eval; token `fnbLsi1FOb9KJfF6A2OBpOPz7FPaBre`, unchanged |
| SURF firewall | 29540–42 open (verified from LUMI) |
| LUMI dclt watcher | armed on uan04 (pid 141016) — leave it |
| wandb | project `pww-diloco-20k-elastic` throughout |

Tokens: full arm **run6** `tqylMnFrpAK0uZOdN5NXSr5iC15MvfJ` (regenerated by the
clean reset — anything still holding the old `SyI7...` token gets 401), dclt
`fnbLsi1FOb9KJfF6A2OBpOPz7FPaBre`. Same `WANDB_API_KEY` everywhere (in
`$S/.pww-secrets` on the VM).

---

## 1. FIRST — the clean full-arm run (run6, from step 0)

The VM side is already done: global model, momentum, checkpoints and DARL
state wiped at 13:43 (the repaired `.venv` preserved); fresh coordinator with
2692 blocks / 0 committed; wandb `central-aggregator-full-run6`. On the sites,
clear leftovers and submit fresh — **FRESH flags ON, run6 names, NEW token**:

```bash
squeue --me | grep titan-full     # scancel any hit: old submits hold the dead SyI7... token
```

```bash
# Snellius -- fresh run6
export WANDB_API_KEY=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
export TOK_FULL=tqylMnFrpAK0uZOdN5NXSr5iC15MvfJ
...
sbatch -J pww-snellius-titan-full --time=24:00:00 \
  --export=ALL,DARL_TOKEN="$TOK_FULL",WANDB_API_KEY="$WANDB_API_KEY",PWW_FRESH_RUN=1,PWW_FRESH_DELETE=1,ENABLE_WANDB=1,WANDB_PROJECT=pww-diloco-20k-elastic,WANDB_RUN_NAME=diloco-snellius-full-run6 \
  scripts/snellius/job_titan_diloco.sh
```

```bash
# LUMI -- follow-watcher, so LUMI starts when Snellius does and every round is k=2.
# (Do NOT touch pww-lumi-central if it reappears.)
cd ~/ProjectWorldWide
export WANDB_API_KEY=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
export TOK_FULL=tqylMnFrpAK0uZOdN5NXSr5iC15MvfJ

nohup scripts/follow_watch.sh \
  --url http://145.38.206.143:29510 --token "$TOK_FULL" --cluster snellius -- \
  sbatch -A project_462000226 -p standard-g -J pww-lumi-titan-full --time=24:00:00 \
    --export=ALL,DARL_TOKEN="$TOK_FULL",WANDB_API_KEY="$WANDB_API_KEY",PWW_FRESH_RUN=1,PWW_FRESH_DELETE=1,ENABLE_WANDB=1,WANDB_PROJECT=pww-diloco-20k-elastic,WANDB_RUN_NAME=diloco-lumi-full-run6 \
    scripts/lumi/job_titan_diloco.sh \
  > watch-full-follow.log 2>&1 &
tail -3 watch-full-follow.log   # real tqyl... token in the echo, and it must NOT fire yet
```

The watcher now requires a LIVE heartbeat (`--fresh-within`, default 300s),
not mere presence: the presence-only version fired instantly against a resumed
coordinator's dead membership history and submitted LUMI hours early. On this
FRESH coordinator (empty history) old and new code behave identically, so an
un-pulled LUMI checkout is safe here — but pull before pointing any watcher at
a resumed stack.

Verify on the VM once both are up:
`grep "cluster(s) \[" $S/runs/central/flower.log | tail -3` — round-1 loss
≈ 9.9, membership `[snellius, lumi]`.
