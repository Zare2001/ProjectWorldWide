...
def train_one_epoch(model, loader, optimizer, scheduler, criterion, info, args,
                    epoch: int, autocast_dtype, metrics) -> dict:
    model.train()
    log = get_logger()
...
def main(argv: list[str] | None = None) -> None:
    ...
    log.info("global batch %d (per-rank %d x %d ranks) | lr %.4f | epochs %d",
             global_batch, args.batch_size, info.world_size, lr, args.epochs)
    ...
    with MetricsWriter(output_dir, info.rank) as metrics:
        for epoch in range(start_epoch, args.epochs):
            ...
            tr = train_one_epoch(model, train_loader, optimizer, scheduler, criterion,
                                 info, args, epoch, autocast_dtype, metrics)
            ...
            if (epoch + 1) % args.save_every == 0 or is_last:
                name = f"step_{epoch + 1}" if args.sharded_checkpoint else f"epoch_{epoch + 1}.pt"
